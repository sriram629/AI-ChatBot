...
export default function MessageImage({ src, alt = 'Chat image' }: { src?: string; alt?: string }) {
  ...
  return <span className="my-3 block max-w-full">
    ...
    <dialog ref={dialog} aria-label={alt} className="fixed inset-0 m-auto max-h-[90dvh] max-w-[95vw] rounded-xl border border-border bg-card p-4 text-foreground backdrop:bg-black/80">
